...
const getServersQuantityByPlan = (planKey: PlanKey | null): number => {
	if (!planKey) return SERVERS_QUANTITY_BY_PLAN.pro
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	if (status === 'AUTHORIZED' || status === 'CONFIRMED') {
		...
		const serversQuantity = getServersQuantityByPlan(planKey ?? 'free')
		console.log(serversQuantity, planKey)
		await db.update(user).set({serversQuantity}).where(eq(user.id, current.userId))
