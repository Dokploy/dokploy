	...
	requestBackupFileRestore: protectedProcedure
		...
		.mutation(async ({ input }) => {
			try {
				...
				const restoreCommand = `rclone backend restore ${rcloneFlags.join(" ")} --include ${shEscape(normalizedPath)} ${shEscape(bucketPath)} -o priority=${shEscape(priority)} -o lifetime=${input.lifetimeDays}`;
