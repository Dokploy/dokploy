...
import { useReadOnly, getReadOnlyProps, getReadOnlyButtonProps } from "@/hooks/use-readonly";
		...
		return (
			<Alert className="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950">
				...
				<AlertDescription className="text-amber-800 dark:text-amber-200">
					<strong>Read-Only Access:</strong> You have view-only permissions for this service. 
					Action buttons are disabled. Contact your administrator for edit permissions.
				</AlertDescription>
			</Alert>
		);
	}
...
export const ReadOnlyIndicator = ({ serviceId, children }: ReadOnlyIndicatorProps) => {
...
export const ReadOnlyNotification = ({ serviceId, serviceName = "service" }: ReadOnlyNotificationProps) => {
	const { isReadOnly } = useReadOnly({ serviceId });
	...
	return (
		<Alert className="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950">
			<Lock className="h-4 w-4 text-amber-600 dark:text-amber-400" />
			<AlertDescription className="text-amber-800 dark:text-amber-200">
				<strong>Read-Only Access:</strong> You have view-only permissions for this {serviceName}. 
				Action buttons are disabled. Contact your administrator for edit permissions.
			</AlertDescription>
		</Alert>
	);
...
export const ReadOnlyFormWrapper = ({ 
	...
	const cloneChildren = (children: ReactNode): ReactNode => {
		return React.Children.map(children, (child) => {
			...
			const componentName = child.type?.displayName || child.type?.name || child.type;
			...
			if (componentName === 'input' || 
				...
				(child.props && (
					...
					(child.props.type === 'radio' && componentName !== 'RadioGroupItem') ||
					(child.props.type === 'checkbox' && componentName !== 'Checkbox') ||
					(child.props.value !== undefined && child.props.type !== 'checkbox') ||
					(child.props.defaultValue !== undefined && child.props.type !== 'checkbox') ||
					(child.props.checked !== undefined && child.props.type !== 'checkbox') ||
					(child.props.onValueChange && child.props.type !== 'checkbox' && componentName !== 'RadioGroupItem') ||
