						...
						<div className="flex flex-col gap-4 w-full">
							...
							>
								<TabsList>
									<TabsTrigger value="monthly">
										{t("settings.billing.tabs.monthly")}
									</TabsTrigger>
									<TabsTrigger value="annual">
										{t("settings.billing.tabs.annual")}
									</TabsTrigger>
								</TabsList>
							</Tabs>
							{admin?.user.stripeSubscriptionId && (
								<div className="space-y-2 flex flex-col">
									...
									<p className="text-sm text-muted-foreground">
										{t("settings.billing.plan.usage", {
											servers: servers ?? 0,
											max: admin?.user.serversQuantity ?? 1,
										})}
									</p>
									<div>
									...
									{admin && admin.user.serversQuantity! <= (servers ?? 0) && (
										<div className="flex flex-row gap-4 p-2 bg-yellow-50 dark:bg-yellow-950 rounded-lg items-center">
											...
											<span className="text-sm text-yellow-600 dark:text-yellow-400">
												{t("settings.billing.plan.limitReached")}
											</span>
										</div>
									)}
								</div>
							...
							<div className="flex flex-col gap-1.5 mt-4">
								<span className="text-base text-primary">
									{t("settings.billing.help.title")}
								</span>
								<span className="text-sm text-muted-foreground">
									{t("settings.billing.help.description")}
								</span>
								<Button className="rounded-full bg-[#5965F2] hover:bg-[#4A55E0] w-fit">
									...
									>
										...
										{t("settings.billing.help.joinDiscord")}
									</Link>
								</Button>
							</div>
							{isLoading ? (
										...
										return (
											<div key={product.id}>
												...
												>
													...
													>
														{[
															"settings.billing.features.allDokploy",
															"settings.billing.features.unlimitedDeployments",
															"settings.billing.features.selfHosted",
															"settings.billing.features.fullAccess",
															"settings.billing.features.integration",
															"settings.billing.features.backups",
															"settings.billing.features.incoming",
														].map((featureKey) => (
															<li
																key={featureKey}
															...
															>
																<CheckIcon />
																<span className="ml-4">{t(featureKey)}</span>
															</li>
														))}
													</ul>
													<div className="flex flex-col gap-2 mt-4">
														<div className="flex items-center gap-2 justify-center">
															<span className="text-sm text-muted-foreground">
																{t("settings.billing.serversLabel", {
																	count: serverQuantity,
																})}
															</span>
														</div>
															...
															{data?.subscriptions?.length === 0 && (
																<div className="justify-end w-full">
																	<Button
																		...
																		disabled={serverQuantity < 1}
																	>
																		{t("settings.billing.button.subscribe")}
																	</Button>
																</div>
															)}
														</div>
