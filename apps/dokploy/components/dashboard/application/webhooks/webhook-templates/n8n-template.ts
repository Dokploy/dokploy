...
export class N8nTemplateGenerator implements TemplateGenerator<N8nWebhookConfig> {
  generate(
    context: BaseWebhookContext,
    config?: N8nWebhookConfig
  ): N8nWebhookPayload {
    const { event, timestamp, webhookId, deployment, entity, project, source, trigger, error } = context;
    ...
    if (config?.transformScript) {
      try {
        const transform = new Function("payload", "context", config.transformScript);
  ...
  private processErrorLogs(logs?: string, maxLines?: number): string | undefined {
  ...
  private generateTags(context: BaseWebhookContext, config?: N8nWebhookConfig): string[] {
    const tags: string[] = [];
  ...
  private meetsSeverityThreshold(event: string, minimumSeverity: "warning" | "error"): boolean {
