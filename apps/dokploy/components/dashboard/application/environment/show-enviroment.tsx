  ...
  : data?.env !== currentEnvironment;
	...
	useEffect(() => {
		if (data) {
			...
			if (type === 'compose' && 'is_secured' in data && data.is_secured === true) {
    ...
    try {
        ...
        if (!isSecureMode) {
            ...
            form.setValue("environment", encryptEnvironmentValues(currentFormValues), { shouldDirty: false });
        } else {
    ...
    const isSecuredFromDB = type === 'compose' && data && 'is_secured' in data && data.is_secured === true;
    
    try {
        // Use the appropriate mutation based on service type
        switch (type) {
            case 'postgres':
                await postgresMutation.mutateAsync({ postgresId: id, env: envToSave });
                break;
            case 'redis':
	...
	const handleCancel = () => {
		if (type === 'compose' && data && 'is_secured' in data && data.is_secured === true) {
				...
				<CardHeader className="flex flex-row w-full items-center justify-between">
					<div>
						...
						<CardDescription>
							...
							{type === 'compose' && data && 'is_secured' in data && data.is_secured === true && (
