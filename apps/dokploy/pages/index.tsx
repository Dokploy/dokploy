...
export default function Home({ IS_CLOUD }: Props) {
	...
	const onSubmit = async (values: LoginForm) => {
		...
		try {
			...
			if (error) {
				...
				setError(error.message || t("index.errorOccurred"));
				return;
			}
			...
			if (data?.twoFactorRedirect as boolean) {
				setTwoFactorCode("");
				setIsTwoFactor(true);
				toast.info(t("index.pleaseEnter2FA"));
				return;
			}
			...
			toast.error(t("index.errorOccurred"));
		} finally {
	...
	const onTwoFactorSubmit = async (e: React.FormEvent) => {
		...
		if (twoFactorCode.length !== 6) {
			toast.error(t("index.pleaseEnterValidCode"));
			return;
		}
		...
		try {
			...
			if (error) {
				toast.error(error.message);
				setError(error.message || t("index.errorOccurred2FA"));
				return;
			}

			toast.success(t("index.loggedInSuccessfully"));
			router.push("/dashboard/projects");
		} catch {
			toast.error(t("index.errorOccurred2FA"));
		} finally {
	...
	const onBackupCodeSubmit = async (e: React.FormEvent) => {
		...
		if (backupCode.length < 8) {
			toast.error(t("index.pleaseEnterValidBackup"));
			return;
		}
		...
		try {
			...
			if (error) {
				toast.error(error.message);
				setError(error.message || t("index.errorOccurredBackup"));
				return;
			}
			...
			toast.error(t("index.errorOccurredBackup"));
		} finally {
	...
	const handleGithubSignIn = async () => {
		...
		try {
			...
			toast.error(t("index.errorOccurredGithub"), {
	...
	const handleGoogleSignIn = async () => {
		...
		try {
			...
			toast.error(t("index.errorOccurredGoogle"), {
				description: error instanceof Error ? error.message : "Unknown error",
			});
		} finally {
						...
						<Form {...loginForm}>
							...
							>
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>{t("index.email")}</FormLabel>
											<FormControl>
												<Input
					...
					<>
						...
						>
							<div className="flex flex-col gap-2">
								<Label>{t("index.twoFactorTitle")}</Label>
								<InputOTP
									value={twoFactorCode}
						...
						>
							<DialogContent>
								<DialogHeader>
									<DialogTitle>{t("index.backupCodeTitle")}</DialogTitle>
									<DialogDescription>
										{t("index.backupCodeDescription")}
									</DialogDescription>
								</DialogHeader>

								<form onSubmit={onBackupCodeSubmit} className="space-y-4">
									<div className="flex flex-col gap-2">
										<Label>{t("index.backupCode")}</Label>
										<Input
											value={backupCode}
											onChange={(e) => setBackupCode(e.target.value)}
											placeholder={t("index.backupCodePlaceholder")}
											className="font-mono"
										/>
										<CardDescription>
											{t("index.backupCodeDescription2")}
										</CardDescription>
									</div>
