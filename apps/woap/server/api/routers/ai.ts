import { IS_CLOUD } from "@woap/server/constants";
import {
	apiCreateAi,
	apiUpdateAi,
	deploySuggestionSchema,
} from "@woap/server/db/schema/ai";
import {
	createDomain,
	createMount,
	findEnvironmentById,
} from "@woap/server/index";
import {
	deleteAiSettings,
	getAiSettingById,
	...
	saveAiSettings,
	suggestVariants,
} from "@woap/server/services/ai";
import { createComposeByTemplate } from "@woap/server/services/compose";
import { findProjectById } from "@woap/server/services/project";
import {
	addNewService,
	checkServiceAccess,
} from "@woap/server/services/user";
import {
	getProviderHeaders,
	getProviderName,
	type Model,
} from "@woap/server/utils/ai/select-ai-provider";
...
import { parseUserIntent } from "@/server/utils/ai-chat-handler";
...
export const aiRouter = createTRPCRouter({
	chat: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			try {
				// Get the first available AI configuration for the organization
				const aiSettings = await getAiSettingsByOrganizationId(
				...
				if (!aiSettings || aiSettings.length === 0) {
					return {
						message:
							"I'd love to help, but you need to configure an AI provider first! Go to Settings > AI to set up OpenAI, Anthropic, or another provider.",
						actions: [],
					};
				}
