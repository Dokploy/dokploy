  ...
  try {
    ...
    for (const line of dfLines) {
      try {
        ...
        const sizeVal = obj.Size || obj.UsedSize || obj.Used || obj.Reclaimable || "0B";
        if (typeVal.toLowerCase().includes("volume") && nameVal) {
          const size = parseDockerHumanSizeToBytes(String(sizeVal).split("(")[0].trim());
    ...
    const psRes = serverId ? await execAsyncRemote(serverId, psCmd) : await execAsync(psCmd);
    const containerLines = (psRes.stdout || "").trim().split("\n").filter(Boolean);
    ...
    for (const line of containerLines) {
      try {
        ...
        const sizeParsed = parseDockerHumanSizeToBytes(sizeStr.split("(")[0].trim());
        if (id) {
